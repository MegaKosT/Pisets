...
def main():
    ...
    waveform, sample_rate = torchaudio.load("./mic008-test_BiSYeroO (online-audio-converter.com).wav")
    resampler = torchaudio.transforms.Resample(sample_rate, 16000)
    resampled_waveform = resampler(waveform)
    

   # audio = datasets.Dataset.from_dict({"audio": ["./mic008-test_BiSYeroO (online-audio-converter.com).wav"]}).cast_column("audio", datasets.Audio())
    ...
    features = processor(resampled_waveform, sampling_rate=16000 ,return_tensors="pt", padding="longest").input_values.to(device)
    feature_lengths = [features.shape[1]]
    feature_lengths = torch.tensor(feature_lengths, device=device)
    # feature_lengths = [f.size(0) for f in features]
    #feature_lengths = features.input_values.shape
