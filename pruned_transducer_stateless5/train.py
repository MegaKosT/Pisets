...
from wav2vec2 import PretrainedWav2Vec2ForTransducer
...
#         type=str,
#         default=None,
#         help="""Path to checkpoint if that exists. 
#         Checkpoint is loads with torch.load and may include epoch, model_state_dict
#         optimizer_state_dict, scheduler, grad_scaler, loss.
#         """,
#     )
    parser.add_argument(
        "--master-port",
...
def run(args):#rank, world_size, args):
    """
    Args:
      rank:
...
#         model = DDP(model, device_ids=[rank])
    ...
    def preproc_val_batch(batch):
        wavs = [audio['array'] for audio in batch['audio']]
    
        processed_audio = processor(wavs, padding=True, return_tensors='pt', sampling_rate=16000)
        input_values = processed_audio.input_values
        attention_mask = processed_audio.attention_mask
